...
public class Util {
    ...
    public static void log(final String event, WorkFlow workFlow) {
        if(event.equals(Constraints.ROLL)){
            logRoll(workFlow);
        }
        if(event.equals(Constraints.BEFORE_MOVE)){
            logBeforeMove(workFlow);
        }
        if(event.equals(Constraints.SHOULD_GO)){
            logShouldGo(workFlow);
        }
        if(event.equals(Constraints.SNAKE)){
            logSnake(workFlow);
        }
        if(event.equals(Constraints.SNAKE_MOVE)){
            logSnakeMove(workFlow);
        }
        if(event.equals(Constraints.LADDER)){
            logLadder(workFlow);
        }
        if(event.equals(Constraints.LADDER_MOVE)){
            logLadderMove(workFlow);
        }
        if(event.equals(Constraints.WIN)){
            logWin(workFlow);
        }
        if(event.equals(Constraints.ON_PLAYER)){
            logOnPlayer(workFlow);
        }
        if(event.equals(Constraints.INVALID_MOVE)){
            logInvalidMove(workFlow);
        }
    }

    private static void logOnPlayer(WorkFlow workFlow) {
        if(workFlow.turnFirstPlayer){
            System.out.println(workFlow.player1.name + " stepped on another player go to (0,0)" 
            +workFlow.newPosition.x + " , "+workFlow.newPosition.y);
        }else{
            System.out.println(workFlow.player2.name + " stepped on another player go to (0,0)"
            +workFlow.newPosition.x + " , "+workFlow.newPosition.y);
        }
    }
    ...
    public static void checkSnake(final Box[][] board, WorkFlow workFlow) {
        ...
        log(Constraints.SNAKE, workFlow);
        workFlow.newPosition = board[workFlow.newPosition.x][workFlow.newPosition.y].snakeTail;
        log(Constraints.SNAKE_MOVE, workFlow);
    }

    public static void onPlayer(final Box[][] board, WorkFlow workFlow){
        
        if (workFlow.player1.position == workFlow.player2.position) {
            boolean start;
            if(workFlow.player1.position == workFlow.player2.position){
                start = true;
                System.out.println("Stepped on another player");
            log(Constraints.ON_PLAYER, workFlow);
            workFlow.newPosition = Box.start;
            log(Constraints.ON_PLAYER, workFlow);
        }
    }
}
    ...
    public static boolean isOverShooting(WorkFlow workFlow) {
        ...
        log(Constraints.INVALID_MOVE, workFlow);
        return true;
    }
